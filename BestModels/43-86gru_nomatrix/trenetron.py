...
stop_early = EarlyStopping(monitor='val_binary_accuracy',
                           patience=30,  # кол-во эпох без улучшений и стоп
...
data = pd.read_csv('../../data/avoska_human.csv', header=None, names=['category', 'text'])
...
# Подготовка тестовых данных (все данные целиком, сырые)
x_data_all = np.asarray(data['text'].tolist())
...
print('Конечный размер базы данных после токенайзера - ', len(data.index))
...
x_train = np.asarray(data['text'].tolist())
y_train = np.asarray(data['category'].tolist())
# x_data = np.asarray(nul_data['text'].tolist() + odin_data['text'].tolist())
# y_data = np.asarray(nul_data['category'].tolist() + odin_data['category'].tolist())
print('Конечный размер базы данных после выравнивания - ', x_train.shape)
